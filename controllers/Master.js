...
export const getProjectNum = async (req, res) =>{
    try {
        ...
        let closed_pn = await pool.query(
          `select count(*) as proj_num from sc_pms.project where status='CLOSE'`
        );
        
        // jumlah project cancel
        let cancel_pn = await pool.query(
          `select count(*) as proj_num from sc_pms.project where status='CANCEL'`
        );
...
export const searchProjects = async (req, res) =>{
  ...
  try {
      let response = await pool.query(
        `select * from sc_pms.project where LOWER(projectname) like '%${keyword}%'`
      );
      res.status(200).json(response.rows);
  } catch (error) {
      res.status(500).json({msg: error.message});
